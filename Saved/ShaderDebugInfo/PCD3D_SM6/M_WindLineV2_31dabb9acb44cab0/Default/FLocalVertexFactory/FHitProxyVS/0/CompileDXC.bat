@ECHO OFF
SET DXC="D:\Software\EpicGames\UE_5.4\Engine\Binaries\ThirdParty\ShaderConductor\Win64\dxc.exe"
IF NOT EXIST %DXC% (
	ECHO Couldn't find dxc.exe under "D:\Software\EpicGames\UE_5.4\Engine\Binaries\ThirdParty\ShaderConductor\Win64"
	GOTO :END
)
%DXC% -HV 2018 -Zpr -O3 -auto-binding-space 0 -Wno-parentheses-equality -disable-lifetime-markers -T vs_6_6 -E Main -Fc HitProxyVertexShader.d3dasm -Fo HitProxyVertexShader.dxil HitProxyVertexShader.usf
:END
PAUSE
